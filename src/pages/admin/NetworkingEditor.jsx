...
const NetworkingEditor = () => {
    ...
    const handleHeroChange = (field, value) => {
        updateContent(`networking.hero.${field}`, value);
    };

    const handleStatChange = (index, field, value) => {
        const newStats = [...(content.networking?.stats || [])];
        newStats[index] = { ...newStats[index], [field]: value };
        updateContent('networking.stats', newStats);
    };

    const handleServiceChange = (index, field, value) => {
        const newServices = [...(content.networking?.services || [])];
        newServices[index] = { ...newServices[index], [field]: value };
        updateContent('networking.services', newServices);
    };
    ...
    const removeService = (index) => {
        const newServices = (content.networking?.services || []).filter((_, i) => i !== index);
        updateContent('networking.services', newServices);
    };
    ...
    const handleSoftwareListChange = (index, field, value) => {
        const newList = [...(content.networking?.softwareList || [])];
        newList[index] = { ...newList[index], [field]: value };
        updateContent('networking.softwareList', newList);
    };

    const addSoftwareItem = () => {
        const newList = [...(content.networking?.softwareList || []), { name: 'New Item', category: 'Category', icon: '📦' }];
        updateContent('networking.softwareList', newList);
    };

    const removeSoftwareItem = (index) => {
        const newList = (content.networking?.softwareList || []).filter((_, i) => i !== index);
        updateContent('networking.softwareList', newList);
    };

    // Gallery handlers
    const handleGalleryChange = (field, value) => {
        updateContent(`networking.gallery.${field}`, value);
    };

    const handleGalleryImageChange = (index, field, value) => {
        const newImages = [...(content.networking?.gallery?.images || [])];
        newImages[index] = { ...newImages[index], [field]: value };
        updateContent('networking.gallery.images', newImages);
    };

    const addGalleryImage = () => {
        const newImages = [...(content.networking?.gallery?.images || []), { url: '', title: 'New Image', description: 'Description' }];
        updateContent('networking.gallery.images', newImages);
    };

    const removeGalleryImage = (index) => {
        const newImages = (content.networking?.gallery?.images || []).filter((_, i) => i !== index);
        updateContent('networking.gallery.images', newImages);
    };

    // Why Choose Us handlers
    const handleWhyChooseUsChange = (field, value) => {
        updateContent(`networking.whyChooseUs.${field}`, value);
    };

    const handleReasonChange = (index, field, value) => {
        const newReasons = [...(content.networking?.whyChooseUs?.reasons || [])];
        newReasons[index] = { ...newReasons[index], [field]: value };
        updateContent('networking.whyChooseUs.reasons', newReasons);
    };

    const addReason = () => {
        const newReasons = [...(content.networking?.whyChooseUs?.reasons || []), { title: 'New Reason', description: 'Description', stat: '0', statLabel: 'Label', color: 'from-blue-500 to-blue-600' }];
        updateContent('networking.whyChooseUs.reasons', newReasons);
    };

    const removeReason = (index) => {
        const newReasons = (content.networking?.whyChooseUs?.reasons || []).filter((_, i) => i !== index);
        updateContent('networking.whyChooseUs.reasons', newReasons);
    };

    const handleFeatureTextChange = (index, value) => {
        const newFeatures = [...(content.networking?.whyChooseUs?.features || [])];
        newFeatures[index] = value;
        updateContent('networking.whyChooseUs.features', newFeatures);
    };

    const addFeatureText = () => {
        const newFeatures = [...(content.networking?.whyChooseUs?.features || []), 'New Feature'];
        updateContent('networking.whyChooseUs.features', newFeatures);
    };

    const removeFeatureText = (index) => {
        const newFeatures = (content.networking?.whyChooseUs?.features || []).filter((_, i) => i !== index);
        updateContent('networking.whyChooseUs.features', newFeatures);
    };

    // Pricing Cards handlers
    const handlePricingCardChange = (index, field, value) => {
        const newCards = [...(content.networking?.pricingCards || [])];
        newCards[index] = { ...newCards[index], [field]: value };
        updateContent('networking.pricingCards', newCards);
    };
    ...
    const removePricingCard = (index) => {
        const newCards = (content.networking?.pricingCards || []).filter((_, i) => i !== index);
        updateContent('networking.pricingCards', newCards);
    };
    ...
    const handleComparisonTableChange = (newData) => {
        updateContent('networking.comparisonTable', newData);
    };
    ...
    return (
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-4xl font-bold text-slate-900 mb-2">Networking Page Editor</h1>
                    <p className="text-slate-600">Manage all networking page content</p>
                </div>
                <button
                    onClick={handleSave}
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            type="text"
                            value={content.networking?.hero?.title || ''}
                    ...
                    <div>
                        ...
                        <textarea
                            value={content.networking?.hero?.subtitle || ''}
                    ...
                    <div>
                        ...
                        <input
                            type="tel"
                            value={content.networking?.hero?.phone || ''}
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {content.networking?.stats?.map((stat, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-6">
                    {content.networking?.services?.map((service, index) => (
                            ...
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        disabled={index === (content.networking?.services || []).length - 1}
                                        className="p-2 text-slate-600 hover:bg-slate-200 rounded disabled:opacity-30"
                                    >
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {content.networking?.softwareList?.map((item, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-4 mb-6">
                    <input
                        type="text"
                        value={content.networking?.gallery?.heading || ''}
                    ...
                    <input
                        type="text"
                        value={content.networking?.gallery?.title || ''}
                    ...
                    <input
                        type="text"
                        value={content.networking?.gallery?.subtitle || ''}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {content.networking?.gallery?.images?.map((image, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-4 mb-6">
                    <input
                        type="text"
                        value={content.networking?.whyChooseUs?.badge || ''}
                    ...
                    <input
                        type="text"
                        value={content.networking?.whyChooseUs?.heading || ''}
                        onChange={(e) => handleWhyChooseUsChange('heading', e.target.value)}
                        className="w-full px-4 py-2 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:outline-none"
                        placeholder="Main heading"
                    />
                    <textarea
                        value={content.networking?.whyChooseUs?.subtitle || ''}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    {content.networking?.whyChooseUs?.reasons?.map((reason, index) => (
                        <div key={index} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <div className="flex items-center justify-between mb-3">
                ...
                <div className="space-y-4">
                    <input
                        type="text"
                        value={content.networking?.whyChooseUs?.ctaHeading || ''}
                        onChange={(e) => handleWhyChooseUsChange('ctaHeading', e.target.value)}
                        className="w-full px-4 py-2 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:outline-none"
                        placeholder="CTA heading"
                    />
                    <textarea
                        value={content.networking?.whyChooseUs?.ctaDescription || ''}
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {content.networking?.whyChooseUs?.features?.map((feature, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-6">
                    {content.networking?.pricingCards?.map((card, index) => (
                        <div key={index} className="p-6 bg-slate-50 rounded-xl border border-slate-200">
                            <div className="flex items-center justify-between mb-4">
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                <ComparisonTableEditor
                    data={content.networking?.comparisonTable}
                ...
                <IconPicker
                    selectedIcon={
                        currentIconField?.type === 'service'
                            ? content.networking.services[currentIconField.index].icon
                            : currentIconField?.type === 'reason'
                                ? content.networking.whyChooseUs.reasons[currentIconField.index].icon
                                : currentIconField?.type === 'software'
                                    ? content.networking.softwareList[currentIconField.index].icon
                                    : ''
                    }
