...
const PrinterEditor = () => {
    ...
    const handleHeroChange = (field, value) => {
        updateContent(`printer.hero.${field}`, value);
    };

    const handleStatChange = (index, field, value) => {
        const newStats = [...(content.printer?.stats || [])];
        newStats[index] = { ...newStats[index], [field]: value };
        updateContent('printer.stats', newStats);
    };

    const handleServiceChange = (index, field, value) => {
        const newServices = [...(content.printer?.services || [])];
        newServices[index] = { ...newServices[index], [field]: value };
        updateContent('printer.services', newServices);
    };
    ...
    const removeService = (index) => {
        const newServices = (content.printer?.services || []).filter((_, i) => i !== index);
        updateContent('printer.services', newServices);
    };
    ...
    const handleSoftwareListChange = (index, field, value) => {
        const newList = [...(content.printer?.softwareList || [])];
        newList[index] = { ...newList[index], [field]: value };
        updateContent('printer.softwareList', newList);
    };

    const addSoftwareItem = () => {
        const newList = [...(content.printer?.softwareList || []), { name: 'New Item', category: 'Category', icon: '📦' }];
        updateContent('printer.softwareList', newList);
    };

    const removeSoftwareItem = (index) => {
        const newList = (content.printer?.softwareList || []).filter((_, i) => i !== index);
        updateContent('printer.softwareList', newList);
    };

    // Gallery handlers
    const handleGalleryChange = (field, value) => {
        updateContent(`printer.gallery.${field}`, value);
    };

    const handleGalleryImageChange = (index, field, value) => {
        const newImages = [...(content.printer?.gallery?.images || [])];
        newImages[index] = { ...newImages[index], [field]: value };
        updateContent('printer.gallery.images', newImages);
    };

    const addGalleryImage = () => {
        const newImages = [...(content.printer?.gallery?.images || []), { url: '', title: 'New Image', description: 'Description' }];
        updateContent('printer.gallery.images', newImages);
    };

    const removeGalleryImage = (index) => {
        const newImages = (content.printer?.gallery?.images || []).filter((_, i) => i !== index);
        updateContent('printer.gallery.images', newImages);
    };

    // Why Choose Us handlers
    const handleWhyChooseUsChange = (field, value) => {
        updateContent(`printer.whyChooseUs.${field}`, value);
    };

    const handleReasonChange = (index, field, value) => {
        const newReasons = [...(content.printer?.whyChooseUs?.reasons || [])];
        newReasons[index] = { ...newReasons[index], [field]: value };
        updateContent('printer.whyChooseUs.reasons', newReasons);
    };

    const addReason = () => {
        const newReasons = [...(content.printer?.whyChooseUs?.reasons || []), { title: 'New Reason', description: 'Description', stat: '0', statLabel: 'Label', color: 'from-blue-500 to-blue-600' }];
        updateContent('printer.whyChooseUs.reasons', newReasons);
    };

    const removeReason = (index) => {
        const newReasons = (content.printer?.whyChooseUs?.reasons || []).filter((_, i) => i !== index);
        updateContent('printer.whyChooseUs.reasons', newReasons);
    };

    const handleFeatureTextChange = (index, value) => {
        const newFeatures = [...(content.printer?.whyChooseUs?.features || [])];
        newFeatures[index] = value;
        updateContent('printer.whyChooseUs.features', newFeatures);
    };

    const addFeatureText = () => {
        const newFeatures = [...(content.printer?.whyChooseUs?.features || []), 'New Feature'];
        updateContent('printer.whyChooseUs.features', newFeatures);
    };

    const removeFeatureText = (index) => {
        const newFeatures = (content.printer?.whyChooseUs?.features || []).filter((_, i) => i !== index);
        updateContent('printer.whyChooseUs.features', newFeatures);
    };

    // Pricing Cards handlers
    const handlePricingCardChange = (index, field, value) => {
        const newCards = [...(content.printer?.pricingCards || [])];
        newCards[index] = { ...newCards[index], [field]: value };
        updateContent('printer.pricingCards', newCards);
    };
    ...
    const removePricingCard = (index) => {
        const newCards = (content.printer?.pricingCards || []).filter((_, i) => i !== index);
        updateContent('printer.pricingCards', newCards);
    };
    ...
    return (
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-4xl font-bold text-slate-900 mb-2">Printer Page Editor</h1>
                    <p className="text-slate-600">Manage all printer page content</p>
                </div>
                <button
                    onClick={handleSave}
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            type="text"
                            value={content.printer?.hero?.title || ''}
                    ...
                    <div>
                        ...
                        <textarea
                            value={content.printer?.hero?.subtitle || ''}
                    ...
                    <div>
                        ...
                        <input
                            type="tel"
                            value={content.printer?.hero?.phone || ''}
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {content.printer?.stats?.map((stat, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-6">
                    {content.printer?.services?.map((service, index) => (
                            ...
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        disabled={index === (content.printer?.services || []).length - 1}
                                        className="p-2 text-slate-600 hover:bg-slate-200 rounded disabled:opacity-30"
                                    >
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {content.printer?.softwareList?.map((item, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-4 mb-6">
                    <input
                        type="text"
                        value={content.printer?.gallery?.heading || ''}
                    ...
                    <input
                        type="text"
                        value={content.printer?.gallery?.title || ''}
                    ...
                    <input
                        type="text"
                        value={content.printer?.gallery?.subtitle || ''}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {content.printer?.gallery?.images?.map((image, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-4 mb-6">
                    <input
                        type="text"
                        value={content.printer?.whyChooseUs?.badge || ''}
                    ...
                    <input
                        type="text"
                        value={content.printer?.whyChooseUs?.heading || ''}
                        onChange={(e) => handleWhyChooseUsChange('heading', e.target.value)}
                        className="w-full px-4 py-2 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:outline-none"
                        placeholder="Main heading"
                    />
                    <textarea
                        value={content.printer?.whyChooseUs?.subtitle || ''}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    {content.printer?.whyChooseUs?.reasons?.map((reason, index) => (
                        <div key={index} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <div className="flex items-center justify-between mb-3">
                ...
                <div className="space-y-4">
                    <input
                        type="text"
                        value={content.printer?.whyChooseUs?.ctaHeading || ''}
                        onChange={(e) => handleWhyChooseUsChange('ctaHeading', e.target.value)}
                        className="w-full px-4 py-2 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:outline-none"
                        placeholder="CTA heading"
                    />
                    <textarea
                        value={content.printer?.whyChooseUs?.ctaDescription || ''}
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {content.printer?.whyChooseUs?.features?.map((feature, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-6">
                    {content.printer?.pricingCards?.map((card, index) => (
                        <div key={index} className="p-6 bg-slate-50 rounded-xl border border-slate-200">
                            <div className="flex items-center justify-between mb-4">
                ...
                <IconPicker
                    selectedIcon={
                        currentIconField?.type === 'service'
                            ? content.printer.services[currentIconField.index].icon
                            : currentIconField?.type === 'reason'
                                ? content.printer.whyChooseUs.reasons[currentIconField.index].icon
                                : currentIconField?.type === 'software'
                                    ? content.printer.softwareList[currentIconField.index].icon
                                    : ''
                    }
