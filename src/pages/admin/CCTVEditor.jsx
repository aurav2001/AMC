...
const CCTVEditor = () => {
    ...
    const handleHeroChange = (field, value) => {
        updateContent(`cctv.hero.${field}`, value);
    };

    const handleStatChange = (index, field, value) => {
        const newStats = [...(content.cctv?.stats || [])];
        newStats[index] = { ...newStats[index], [field]: value };
        updateContent('cctv.stats', newStats);
    };

    const handleServiceChange = (index, field, value) => {
        const newServices = [...(content.cctv?.services || [])];
        newServices[index] = { ...newServices[index], [field]: value };
        updateContent('cctv.services', newServices);
    };
    ...
    const removeService = (index) => {
        const newServices = (content.cctv?.services || []).filter((_, i) => i !== index);
        updateContent('cctv.services', newServices);
    };
    ...
    const handleSoftwareListChange = (index, field, value) => {
        const newList = [...(content.cctv?.softwareList || [])];
        newList[index] = { ...newList[index], [field]: value };
        updateContent('cctv.softwareList', newList);
    };

    const addSoftwareItem = () => {
        const newList = [...(content.cctv?.softwareList || []), { name: 'New Item', category: 'Category', icon: '📦' }];
        updateContent('cctv.softwareList', newList);
    };

    const removeSoftwareItem = (index) => {
        const newList = (content.cctv?.softwareList || []).filter((_, i) => i !== index);
        updateContent('cctv.softwareList', newList);
    };

    // Gallery handlers
    const handleGalleryChange = (field, value) => {
        updateContent(`cctv.gallery.${field}`, value);
    };

    const handleGalleryImageChange = (index, field, value) => {
        const newImages = [...(content.cctv?.gallery?.images || [])];
        newImages[index] = { ...newImages[index], [field]: value };
        updateContent('cctv.gallery.images', newImages);
    };

    const addGalleryImage = () => {
        const newImages = [...(content.cctv?.gallery?.images || []), { url: '', title: 'New Image', description: 'Description' }];
        updateContent('cctv.gallery.images', newImages);
    };

    const removeGalleryImage = (index) => {
        const newImages = (content.cctv?.gallery?.images || []).filter((_, i) => i !== index);
        updateContent('cctv.gallery.images', newImages);
    };

    // Why Choose Us handlers
    const handleWhyChooseUsChange = (field, value) => {
        updateContent(`cctv.whyChooseUs.${field}`, value);
    };

    const handleReasonChange = (index, field, value) => {
        const newReasons = [...(content.cctv?.whyChooseUs?.reasons || [])];
        newReasons[index] = { ...newReasons[index], [field]: value };
        updateContent('cctv.whyChooseUs.reasons', newReasons);
    };

    const addReason = () => {
        const newReasons = [...(content.cctv?.whyChooseUs?.reasons || []), { title: 'New Reason', description: 'Description', stat: '0', statLabel: 'Label', color: 'from-blue-500 to-blue-600' }];
        updateContent('cctv.whyChooseUs.reasons', newReasons);
    };

    const removeReason = (index) => {
        const newReasons = (content.cctv?.whyChooseUs?.reasons || []).filter((_, i) => i !== index);
        updateContent('cctv.whyChooseUs.reasons', newReasons);
    };

    const handleFeatureTextChange = (index, value) => {
        const newFeatures = [...(content.cctv?.whyChooseUs?.features || [])];
        newFeatures[index] = value;
        updateContent('cctv.whyChooseUs.features', newFeatures);
    };

    const addFeatureText = () => {
        const newFeatures = [...(content.cctv?.whyChooseUs?.features || []), 'New Feature'];
        updateContent('cctv.whyChooseUs.features', newFeatures);
    };

    const removeFeatureText = (index) => {
        const newFeatures = (content.cctv?.whyChooseUs?.features || []).filter((_, i) => i !== index);
        updateContent('cctv.whyChooseUs.features', newFeatures);
    };

    // Pricing Cards handlers
    const handlePricingCardChange = (index, field, value) => {
        const newCards = [...(content.cctv?.pricingCards || [])];
        newCards[index] = { ...newCards[index], [field]: value };
        updateContent('cctv.pricingCards', newCards);
    };
    ...
    const removePricingCard = (index) => {
        const newCards = (content.cctv?.pricingCards || []).filter((_, i) => i !== index);
        updateContent('cctv.pricingCards', newCards);
    };
    ...
    const handleComparisonTableChange = (newData) => {
        updateContent('cctv.comparisonTable', newData);
    };
    ...
    return (
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-4xl font-bold text-slate-900 mb-2">CCTV Page Editor</h1>
                    <p className="text-slate-600">Manage all CCTV page content</p>
                </div>
                <button
                    onClick={handleSave}
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea
                            value={content.cctv?.hero?.subtitle || ''}
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {content.cctv?.stats?.map((stat, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-6">
                    {content.cctv?.services?.map((service, index) => (
                            ...
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        disabled={index === (content.cctv?.services || []).length - 1}
                                        className="p-2 text-slate-600 hover:bg-slate-200 rounded disabled:opacity-30"
                                    >
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {content.cctv?.softwareList?.map((item, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-4 mb-6">
                    <input
                        type="text"
                        value={content.cctv?.gallery?.heading || ''}
                    ...
                    <input
                        type="text"
                        value={content.cctv?.gallery?.title || ''}
                    ...
                    <input
                        type="text"
                        value={content.cctv?.gallery?.subtitle || ''}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {content.cctv?.gallery?.images?.map((image, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-4 mb-6">
                    <input
                        type="text"
                        value={content.cctv?.whyChooseUs?.badge || ''}
                    ...
                    <input
                        type="text"
                        value={content.cctv?.whyChooseUs?.heading || ''}
                        onChange={(e) => handleWhyChooseUsChange('heading', e.target.value)}
                        className="w-full px-4 py-2 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:outline-none"
                        placeholder="Main heading"
                    />
                    <textarea
                        value={content.cctv?.whyChooseUs?.subtitle || ''}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    {content.cctv?.whyChooseUs?.reasons?.map((reason, index) => (
                        <div key={index} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                            <div className="flex items-center justify-between mb-3">
                ...
                <div className="space-y-4">
                    <input
                        type="text"
                        value={content.cctv?.whyChooseUs?.ctaHeading || ''}
                        onChange={(e) => handleWhyChooseUsChange('ctaHeading', e.target.value)}
                        className="w-full px-4 py-2 border-2 border-slate-200 rounded-lg focus:border-blue-500 focus:outline-none"
                        placeholder="CTA heading"
                    />
                    <textarea
                        value={content.cctv?.whyChooseUs?.ctaDescription || ''}
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {content.cctv?.whyChooseUs?.features?.map((feature, index) => (
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                ...
                <div className="space-y-6">
                    {content.cctv?.pricingCards?.map((card, index) => (
                        <div key={index} className="p-6 bg-slate-50 rounded-xl border border-slate-200">
                            <div className="flex items-center justify-between mb-4">
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
                <ComparisonTableEditor
                    data={content.cctv?.comparisonTable}
                ...
                <IconPicker
                    selectedIcon={
                        currentIconField?.type === 'service'
                            ? content.cctv.services[currentIconField.index].icon
                            : currentIconField?.type === 'reason'
                                ? content.cctv.whyChooseUs.reasons[currentIconField.index].icon
                                : currentIconField?.type === 'software'
                                    ? content.cctv.softwareList[currentIconField.index].icon
                                    : ''
                    }
